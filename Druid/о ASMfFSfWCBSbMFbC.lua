-=A+SMFSW+CBSM+RFFbRC=-v1.4,1
-=Druid multi-attack=--=Attack+Swarm+Moonfire+Faerie+StarFire+Wrath+Charge+Bite+Savage Bite+Swipe+Maul+Rip+Fury+Ferocious Bite+Rake+Claw=-
...
-- Проверка, находится ли цель или игрок в бою
local function inCombat(target) 
	if not target then 
		target="player";
	end;
...
-- Использование, проверка наличия или получение перезарядки заклинания (возвращяет sw= true-наличие, 0-перезарядка,self-каст на себя, nil-обычное использование)
...
-- Проверка текущей формы друида(по названию)
local function Aura(ID)
	local ix,icon,name,active,castable,spellId=GetNumShapeshiftForms()
	while(ix>0 and active==nil) do 
		icon,name,active,castable,spellId=GetShapeshiftFormInfo(ix) ix=ix-1;
	end;
	if active then 
		if not ID then 
			return name;
		else 
			return string.find(name,ID);
		end;
	end;
end;
...
function Attack_Actions() 
	...
	-- Получение процента (дробь) здоровья цели
    ...
    -- Логика для формы совы или отсутствия формы
