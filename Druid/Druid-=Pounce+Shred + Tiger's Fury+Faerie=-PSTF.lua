...
function Attack_Actions() 
	...
	if GotBuff("Ability_Druid_CatForm") then 
		...
		if GotBuff("Ability_Ambush") then 
			if UnitCreatureType("target")~="Elemental" and UnitCreatureType("target")~="Mechanical" and UnitCreatureType("target")~="Uncategorized" and UnitCreatureType("target")~="Undead" then 
				if (PM>=50 or OoC) and ActSpell("Pounce") then return; end;
			else 
