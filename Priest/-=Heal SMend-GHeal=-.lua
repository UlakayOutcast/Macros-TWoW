	...
	while(ix>0) do 
		if ix==0 then return nil;
			...
			if spellName==spell and (spellRank==rank or rank==nil) then 
				...
				if sw==0 then print("0")return GetSpellCooldown(ix,"spell");end;
				if sw==nil then 
...
local function CastSpels()
	...
	if not IsAltKeyDown() or IF then 
		if GotBuff("Spell_Shadow_Shadowform") and ActSpell("Shadow Mend",nil,0)==0 then CastSpellByName("Shadow Mend");
		else 
