...
function Attack_Actions() 
	...
	if UnitExists("pettarget") and not GotDebuff("Spell_Shadow_Teleport","player") and ActSpell("Kill Command") then return;end;
	if melee() then 
	...
	if ranged() then 
		...
		if itemtype == "Thrown" then 
			...
			if UnitPowerType("target")==0 and UnitMana("target")/UnitManaMax>0.1 then 
