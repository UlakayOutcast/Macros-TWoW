...
function Attack_Actions() 
	...
	if melee() and mana>0.1 then 
		...
		if VengefulStrikes>=1 and not GotBuff("HolyNova") then action("Holy Strike");return;
			...
			local CS=0; if GotBuff("CrusaderStrike") then CS=GotBuff("CrusaderStrike");end;
			if random(0,CS+1)<=2-GotBuff("MindVision") then if action("Crusader Strike") then return;end; else if action("Holy Strike") then return;end;end;
		end;
	end;
	if melee() and mana>0.1 then 
