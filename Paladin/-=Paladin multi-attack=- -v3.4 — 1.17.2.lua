...
function Attack_Actions() 
	...
	if melee() and mana>0.1 then 
		local CS; if GotBuff("CrusaderStrike") then CS=GotBuff("CrusaderStrike"); else CS=0;end;
		if GotBuff("AuraOfLight") then 
			if math.random(0,CS+1)<=1 then action("Crusader Strike") else action("Holy Strike"); end;
		else 
			if math.random(0,CS+1)==0 then action("Crusader Strike") else action("Holy Strike"); end;
		end;
	end;
	if melee() and mana>0.1 then 
